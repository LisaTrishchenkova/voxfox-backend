...
public class CourseService : ICourseService
{
	...
	public async Task<CourseDto> CreateCourseAsync(CreateCourseDto createCourseDto)
	{
		if (createCourseDto.CategoryId.HasValue)
		{
			...
			if (!categoryExists) throw new System.Exception($"Категория с Id: {createCourseDto.CategoryId} не найдена");
		}
		...
		if (createCourseDto.AuthorId.HasValue)
		{
			...
			if (!authorExists) throw new System.Exception($"Автор с Id: {createCourseDto.AuthorId} не найден");
		}
		...
		if (createdCourse == null) throw new System.Exception("Не удалось добавить курс");
	...
	public async Task<PaginatedResponse<CourseDto>> SearchAsync(CourseSearchRequest request)
	{
		try
		{
			if (!string.IsNullOrWhiteSpace(request.SearchTerm))
				return await _courseSearchService.SearchCoursesAsync(request);

			var query = _courseRepository.GetPublishedCoursesQuery();

			if (request.CategoryId.HasValue) query = query.Where(c => c.CategoryId == request.CategoryId.Value);
			if (!string.IsNullOrWhiteSpace(request.SearchTerm)) query = ApplySearchPriority(query, request.SearchTerm);

			var totalCount = await _courseRepository.GetTotalCountAsync(query);

			if (request.SortBy.HasValue) query = ApplySorting(query, request.SortBy.Value, request.SearchTerm);
	...
	private IQueryable<Models.Entities.Course> ApplySearchPriority(IQueryable<Models.Entities.Course> query,
		string searchTerm)
	{
	...
	private IQueryable<Models.Entities.Course> ApplySorting(IQueryable<Models.Entities.Course> query,
		CoursesSortBy sortBy, string? searchTerm)
	{
	...
	private IQueryable<Models.Entities.Course> ApplyRelevanceSorting(IQueryable<Models.Entities.Course> query,
		string? searchTerm)
	{
